...
class PortfolioApp:
    ...
    def display_welcome_message(self):
        st.markdown(
            "<h3 style='text-align: center;'> Welcome to my portfolio!</h3>", unsafe_allow_html=True)
    ...
    def display_about_me(self):
        ...
        with col2:
            st.markdown(
                "<h3 style='text-align: center;'> Bjørn-Magne Kristensen</h3>", unsafe_allow_html=True)
    ...
    def display_project_list(self):
        st.markdown(
            "<h3 style='text-align: center;'> Project List:</h3>", unsafe_allow_html=True)
        ...
        for i, project in enumerate(self.projects):
            with col1 if i % 2 == 0 else col2:
                ...
                st.markdown(
                    f"<h3 style='text-align: center;'>{project['title']}</h3>", unsafe_allow_html=True)
                st.markdown(
                    f"<p style='text-align: center;'>{description}</p>", unsafe_allow_html=True)
                st.image(image_path, use_column_width=True)
                button_label = f"Go to source code"
                st.markdown(
                    f"<div style='text-align: center;'><a href='{project['url']}' target='_blank'>{button_label}</a></div>", unsafe_allow_html=True)
